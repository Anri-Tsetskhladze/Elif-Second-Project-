...
const ProfileScreen = () => {
  ...
  const { currentUser, isLoading: userLoading, refetch: refetchUser } = useCurrentUser();
  const [activeTab, setActiveTab] = useState<ProfileTabId>("posts");

  // Fetch user stats
  const { data: stats } = useQuery({
    queryKey: ["user-stats", currentUser?._id],
    queryFn: () => userApi.getUserStats(api, currentUser?._id),
  ...
  const {
    ...
    queryKey: ["user-posts", currentUser?.username],
    queryFn: async ({ pageParam = 1 }) => {
      const res = await forumApi.getUserPosts(api, currentUser?.username || "", {
        page: pageParam,
        limit: 20,
      });
      return res.data;
    ...
    enabled: !!currentUser?.username && activeTab === "posts",
  });
  ...
  const {
    ...
    queryKey: ["user-reviews", currentUser?._id],
    queryFn: async ({ pageParam = 1 }) => {
      const res = await reviewApi.getByUser(api, currentUser?._id || "", {
        page: pageParam,
        limit: 20,
      });
      return res.data;
    ...
    enabled: !!currentUser?._id && activeTab === "reviews",
  });
  ...
  const {
    ...
    queryKey: ["user-notes", currentUser?._id],
    queryFn: async ({ pageParam = 1 }) => {
      const res = await noteApi.getByUser(api, currentUser?._id || "", {
        page: pageParam,
        limit: 20,
      });
      return res.data;
    ...
    enabled: !!currentUser?._id && activeTab === "notes",
  });
  ...
  const {
    ...
    queryKey: ["user-saved"],
    queryFn: async ({ pageParam = 1 }) => {
      const res = await forumApi.getBookmarks(api, {
        page: pageParam,
        limit: 20,
      });
      return res.data;
  ...
  const currentData = useMemo(() => {
    switch (activeTab) {
      ...
      case "saved":
        return savedData?.pages.flatMap((p) => p.posts || p.bookmarks || []) || [];
      default:
        return [];
    }
  }, [activeTab, postsData, reviewsData, notesData, savedData]);
  ...
  }, [activeTab, refetchUser, refetchPosts, refetchReviews, refetchNotes, refetchSaved]);
  ...
  const handleLoadMore = useCallback(() => {
    switch (activeTab) {
      ...
      case "notes":
        ...
        break;
      case "saved":
        if (hasMoreSaved && !fetchingMoreSaved) fetchMoreSaved();
        break;
    }
  }, [
    activeTab,
  ...
  }, [currentUser, stats, activeTab, router]);
  ...
  const renderFooter = useCallback(() => {
    const isLoading =
      fetchingMorePosts || fetchingMoreReviews || fetchingMoreNotes || fetchingMoreSaved;
    if (isLoading) {
      return (
        <View className="py-4">
  ...
  if (!currentUser) {
    return (
      <SafeAreaView className="flex-1 bg-white items-center justify-center">
        ...
        <Text className="text-gray-400 text-lg mt-4">Error loading profile</Text>
      </SafeAreaView>
    );
  }
